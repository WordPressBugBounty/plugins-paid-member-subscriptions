...
/**
 * Renders an Active/Inactive status field inside a CPT submit box.
 *
 * @param array $args Status field configuration.
 *
 * @return void
 */
function pms_render_submitbox_status_field( $args ) {
    ...
    if ( empty( $args['label'] ) || empty( $args['select_id'] ) || empty( $args['select_name'] ) ) {
        return;
    }

    echo '<div class="misc-pub-section pms-submitbox-status ' . esc_attr( $args['wrapper_class'] ) . '">';
        echo '<label for="' . esc_attr( $args['select_id'] ) . '"><strong>' . esc_html( $args['label'] ) . '</strong></label>';
        echo '<select id="' . esc_attr( $args['select_id'] ) . '" name="' . esc_attr( $args['select_name'] ) . '">';
            echo '<option value="active" ' . selected( 'active', $args['current_status'], false ) . '>' . esc_html__( 'Active', 'paid-member-subscriptions' ) . '</option>';
            echo '<option value="inactive" ' . selected( 'inactive', $args['current_status'], false ) . '>' . esc_html__( 'Inactive', 'paid-member-subscriptions' ) . '</option>';
        echo '</select>';
        ...
        if ( is_callable( $args['after_callback'] ) ) {
            call_user_func( $args['after_callback'] );
        }
    echo '</div>';
...
/**
 * Returns the markup generated by Subscription Plan integration hooks.
 *
 * @param int $plan_id Subscription Plan ID.
 *
 * @return string
 */
function pms_get_subscription_plan_integrations_markup( $plan_id ) {

    static $cache = array();

    if ( ! isset( $cache[ $plan_id ] ) ) {
        ob_start();
        do_action( 'pms_view_meta_box_subscription_details_bottom', $plan_id );
        do_action( 'pms_subscription_plan_integrations_top', $plan_id );
        do_action( 'pms_subscription_plan_integrations_bottom', $plan_id );
        $cache[ $plan_id ] = ob_get_clean();
    }
...
function pms_output_modal_create_pricing_page(){
    ...
    <div id="pms-create-pricing-page-modal" class="pms-modal cozmoslabs-wrap pms-create-pricing-page-modal <?php if( $pagenow === 'admin.php' && isset( $_GET['page'] ) && $_GET['page'] === 'pms-dashboard-page' ) { echo 'pms-modal-dashboard'; } ?>">
        <div class="pms-modal__holder">
            ...
            <div class="pms-content">
                ...
                ?>
                <p class="pms-modal-intro"><?php esc_html_e( 'Select the subscription plan(s) you want to use to generate a pricing page. You can choose a maximum of 3 plans.', 'paid-member-subscriptions' ); ?></p>
                <form action="<?php echo  esc_url( admin_url( 'admin-post.php') ); ?>" method="post" class="pms-form">
                    ...
                    <p class="cozmoslabs-description pms-pricing-page-style-description"><?php esc_html_e( 'Choose a style that better suits your pricing page.', 'paid-member-subscriptions' ); ?></p>
                    ...
                    <div class="pms-create-pricing-page-actions">
...
function pms_output_modal_style_pricing_page(){
    ...
    <div id="" class="pms-modal cozmoslabs-wrap">
        <div class="pms-modal__holder">
