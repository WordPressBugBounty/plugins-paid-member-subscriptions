...
function pms_add_plugin_marketing_notification( $notification_id = '', $notification_message = '', $notification_class = 'update-nag', $count_in_menu = true, $count_in_submenu = array(), $show_in_all_backend = false ) {

	$notifications = PMS_Plugin_Notifications::get_instance();
	$notifications->add_marketing_notification( $notification_id, $notification_message, $notification_class, $count_in_menu, $count_in_submenu, $show_in_all_backend );
...
function pms_add_plugin_notification_new_add_on() {
    ...
    //pms_add_plugin_marketing_notification( $notification_id, $message, 'pms-notice pms-narrow notice notice-info', true, array( 'pms-addons-page' ) );
    ...
    // Per-seat pricing for Group Memberships (existing add-on users)
    if( apply_filters( 'pms_add_on_is_active', false, 'pms-add-on-group-memberships/index.php' ) ) {

        $notification_id = 'pms_feature_group_memberships_per_seat';
        $message = '<img style="float: left; margin: 20px 12px 10px 0; max-width: 80px;" src="' . PMS_PLUGIN_DIR_URL . 'assets/images/addons/pms-add-on-group-memberships-logo.png" />';
        $message .= '<p style="margin-top: 16px;">' . wp_kses_post( sprintf( '<strong>New in Group Memberships: Per-seat pricing</strong><br>You can now set a price per seat on a Group plan. Members choose how many seats they need at checkout and pay that price times the number of seats.<br>Open a Group plan under <a href="%s">Subscription Plans</a> and set the Pricing Model to <strong>Per seat</strong>.', esc_url( admin_url( 'edit.php?post_type=pms-subscription' ) ) ) ) . '</p>';
        $message .= '<p><a href="'. admin_url( 'edit.php?post_type=pms-subscription' ) .'" class="button-primary">' . esc_html__( 'Subscription Plans', 'paid-member-subscriptions' ) . '</a><a href="https://www.cozmoslabs.com/docs/paid-member-subscriptions/add-ons/group-memberships/?utm_source=pms-plugin-notification&utm_medium=client-site&utm_campaign=pms-group-memberships-per-seat" class="button-secondary" target="_blank">' . esc_html__( 'Learn More', 'paid-member-subscriptions' ) . '</a></p>';
        $message .= '<a href="' . esc_url( wp_nonce_url( add_query_arg( array( 'pms_dismiss_admin_notification' => $notification_id ) ), 'pms_plugin_notice_dismiss' ) ) . '#pms-addons-title" type="button" class="notice-dismiss"><span class="screen-reader-text">' . esc_html__( 'Dismiss this notice.', 'paid-member-subscriptions' ) . '</span></a>';
