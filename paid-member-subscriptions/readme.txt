...
== Changelog ==
= 3.0.1 =
* Fix: Some false positive Stuck Payments issue reports generated by the new Dashboard Health Functionality
* Misc: Compatibility changes for new Advanced Subscription Toolkit option

= 3.0.0 =
Version 3.0.0 of Paid Member Subscriptions is here!

This release bumps the plugin to a new major version to reflect the cumulative improvements and updates made over the past year. It does not introduce any breaking changes.

**Features:**
* Subscriptions page - added a new back-end page which lists all the subscriptions that are available on the website with support for Bulk Cancel, Bulk Delete Subscriptions actions
* Health Monitoring System - added a new widget on the Dashboard which shows admins the overall health of the plugin. If the plugin detects any issues with the overall system, you will be notified here
* Admin Bar Widget - added a new admin bar widget which can be used to quickly access the most relevant plugin pages and it will also notify you of any issues generated by the Health Monitoring System
* Scheduled Payments Overview - added a new widget on the Dashboard page which shows the amount of payments and revenue that will be processed over certain periods: next run, this month, next month, this year

**Fixes:**
* Refactor message handling in payment success functions to ensure proper sanitization and escaping of output. Thanks to loris4py and the Patchstack team
* Fixed a content restriction issue with WooCommerce Products and block themes
* Fixed issue with the GDPR delete button not working correctly on secondary languages with the TranslatePress plugin
* A notice being generated from the Bulk Upload Discount Codes functionality
* Refactor price calculation in Stripe gateway to ensure proper handling of custom prices and sign-up fees

**Misc:**
* Divi 5 compatibility fix
* A notice that could be triggered when receiving PayPal Webhooks
* Back-end UI changes
